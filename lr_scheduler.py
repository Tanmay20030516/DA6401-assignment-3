...
class NoamScheduler(LRScheduler):
    ...
    for epoch in range(num_epochs):
         for batch in train_loader:
            ...
            loss.backward()
            optimizer.step()
            scheduler.step() # after weight update
    ```
    Args:
    ...
    def _get_lr_scale(self) -> float:
        ...
        step = max(1, step)  # step >= 1
        warmup_term = step * (self.warmup_steps**-1.5)
        decay_term = step**-0.5
        # during warmup, warmup_term is smaller; after, decay_term is smaller
        min_term = min(warmup_term, decay_term)
